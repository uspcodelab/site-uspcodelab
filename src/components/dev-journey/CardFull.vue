<template>
  <div class="max-w-sm rounded overflow-hidden shadow-lg">
    ...
    <div class="px-6 py-4">
      <h2 class="font-bold text-3xl lg:text-base text-center mb-2">
        {{ data.title }}
      </h2>
      <p
        class="text-grey-darker text-base lg:text-xs text-center"
        v-html="data.summary"
      />
    </div>
    <div class="px-6 py-4 flex flex-wrap justify-center items-center">
      <span
        v-for="(tech, key, index) in data.technologies"
        :key="index"
        :class="[customClass(tech.name), baseTagClasses]"
      >
        <font-awesome-icon
          v-if="tech.iconName"
          :icon="{ prefix: tech.iconPrefix, iconName: tech.iconName }"
          :aria-labelledby="tech.ariaLabelledBy"
